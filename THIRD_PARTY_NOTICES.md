# Ressources et logiciels tiers

Ce document recense les principales ressources tierces utilisées par Trackheim. Les marques, règles et données de jeu ne sont pas couvertes par la licence MIT du code de l'application.

## Univers et terminologie

Trackheim est un projet fan non officiel, sans affiliation ni approbation de Games Workshop Limited. Mordheim et les noms associés restent la propriété de leurs titulaires respectifs.

## Ressources visuelles

- Corbeau : Brett Croft, distribué par [FreePNGimg](https://freepngimg.com/png/108894-pic-bird-raven-download-hq) sous [CC BY-NC 4.0](https://creativecommons.org/licenses/by-nc/4.0/). La teinte a été adaptée pour l'interface. Cette ressource n'autorise pas un usage commercial.
- Les autres visuels propres à l'interface (`og.png`, `ui-skel.png` et `ui-skull-ribbon.png`) ne proviennent pas de publications, logos ou photographies de produits Games Workshop.

## Icônes et typographie

- [Lucide](https://lucide.dev/) : licence ISC.
- Geist et Geist Mono : SIL Open Font License 1.1.
- IM FELL English : SIL Open Font License 1.1.
- Pirata One : SIL Open Font License 1.1.

## Bibliothèques logicielles

React, Vite, Base UI, cmdk et les autres dépendances installées conservent leurs licences respectives, reproduites dans leurs paquets de distribution.
